...
export const Panel = ({ title, children, className = '', glowColor = 'red' }: PanelProps) => {
  ...
  return (
    <div
      className={`panel-3d backdrop-blur-xl bg-gradient-to-br from-black/90 to-black/70 border ${selectedBorder} rounded-lg overflow-hidden transition-all duration-300 ${
        isHovered ? selectedGlow.hoverShadow : selectedGlow.shadow
      } ${className}`}
    ...
    >
      <div className="panel-header bg-gradient-to-r from-gray-900/50 to-black/50 border-b border-red-900/30 px-4 py-2.5 flex items-center justify-between backdrop-blur-md">
        <div className="flex items-center space-x-3">
          <div className={`w-3 h-3 rounded-full animate-pulse transition-all ${
            isHovered
              ? 'bg-red-400 shadow-[0_0_15px_rgba(239,68,68,1)]'
              : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]'
          }`}></div>
          <span className={`text-red-400 font-mono text-sm tracking-wider transition-colors ${
            isHovered ? 'text-red-300' : ''
          }`}>{title}</span>
        </div>
        <div className="flex space-x-2">
          ...
          <div className="w-3 h-3 rounded-full bg-gray-700 hover:bg-red-500 transition-colors cursor-pointer"></div>
        </div>
      </div>
