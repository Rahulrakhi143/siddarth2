...
import { 
  ...
  RefreshCw, Search, Star, Home, Terminal, X, 
  FileText, Image, Code, Database, Music, Video,
  Globe, Settings, Package, Type, Binary, Save,
  RotateCcw, Eye, EyeOff, Lock, Unlock, Upload,
  ExternalLink, Heart, Pin, History, Filter,
  Grid3x3, List, MoreVertical, Clock, Info,
  ArrowUpRight, FolderOpen, Maximize2, Minimize2,
  FolderTree, FolderInput, FilePlus, FolderPlus as FolderPlusIcon,
  Tag, Bookmark, Users, Share2, Zap, Cpu
} from 'lucide-react';
...
export const FileExplorer = () => {
  ...
  const getAllVisibleNodes = (): any[] => {
    ...
    const traverse = (nodes: any[], depth: number = 0) => {
      nodes.forEach(node => {
        ...
        if (filterType === 'folders' && node.type !== 'directory') return;
  ...
  const handleRename = (nodeId: string) => {
    ...
    if (state.activeFile?.id === nodeId) {
      setActiveFile({ ...state.activeFile, name: editingName });
    }
    ...
    return (
      <div key={node.id}>
        <div
          ...
          className={`flex items-center space-x-2 px-2 py-1.5 cursor-pointer transition-all group ${
            isSelected 
          ...
          {isEditing ? (
            ...
            <div className="flex-1 min-w-0">
              <span className={`text-sm font-mono truncate flex items-center gap-1 ${
                isActiveFile ? 'text-blue-300' : 'text-gray-300'
              }`}>
                {node.name}
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-gray-900 to-black">
      {/* Header */}
      <div className="p-3 border-b border-gray-800 flex items-center justify-between bg-gray-900/80 backdrop-blur-sm">
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2">
            <FolderTree size={16} className="text-blue-400" />
            <span className="text-sm text-gray-300 font-mono">File Explorer</span>
            <div className="text-xs text-blue-500 px-2 py-0.5 bg-blue-900/30 rounded border border-blue-700/50">
              v2.0
            </div>
          </div>
          
          {/* Breadcrumb Navigation */}
          <div className="flex items-center space-x-1 text-xs bg-gray-900/50 rounded-lg px-3 py-1.5 border border-gray-800">
            <button
            ...
            {breadcrumb.map((part, index) => (
              <div key={index} className="flex items-center">
                <button
                  ...
                  className="text-gray-300 hover:text-blue-400 transition-colors px-1 py-0.5 rounded hover:bg-gray-800"
                >
                  {part}
                </button>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="p-2 hover:bg-gray-800 rounded transition-colors text-gray-400 hover:text-gray-300"
            title="Toggle Info Panel (Ctrl+S)"
          >
            <Info size={14} />
          </button>
          ...
          <div className="relative group">
            ...
            {showQuickActions && (
              <div className="absolute right-0 top-full mt-1 bg-gray-900 border border-gray-700 rounded-lg shadow-lg py-1 min-w-48 z-10">
                <button
                ...
                >
                  <Minimize2 size={14} />
                  <span>{viewMode === 'compact' ? "Normal View" : "Compact View"}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-300 placeholder-gray-500 focus:outline-none focus:border-blue-500 w-64"
            />
            {searchQuery && (
              <button
      ...
      <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between bg-gray-900/60">
        <div className="flex items-center space-x-2">
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-blue-900/40 hover:bg-blue-800/40 rounded-lg text-sm text-blue-300 transition-colors"
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm text-gray-300 transition-colors"
          >
          ...
          <button
            onClick={handleUpload}
            className="flex items-center space-x-2 px-4 py-2 bg-purple-900/40 hover:bg-purple-800/40 rounded-lg text-sm text-purple-300 transition-colors"
          >
            <Upload size={14} />
            <span>Upload</span>
          </button>
          ...
          {multiSelect.size > 0 && (
            <div className="flex items-center space-x-2 ml-2 pl-2 border-l border-gray-700">
              <span className="text-xs text-gray-400">{multiSelect.size} selected</span>
              <button
                onClick={() => {
                  selectedNodes.forEach(node => node && handleDelete(node.id));
                  setMultiSelect(new Set());
                }}
                className="px-3 py-1.5 bg-red-900/30 hover:bg-red-800/40 rounded text-xs text-red-300"
              >
                Delete All
              </button>
              <button
                ...
                className="px-3 py-1.5 bg-green-900/30 hover:bg-green-800/40 rounded text-xs text-green-300"
              >
                Download All
              </button>
            </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-gray-800 border border-gray-700 rounded text-xs text-gray-300 focus:outline-none focus:border-blue-500"
            >
          ...
          <div className="flex items-center space-x-1">
            <button
              ...
              className={`p-2 rounded ${viewMode === 'list' ? 'bg-blue-900/40 text-blue-300' : 'hover:bg-gray-800 text-gray-500'}`}
              title="List View"
            >
              <List size={14} />
            </button>
            <button
              ...
              className={`p-2 rounded ${viewMode === 'grid' ? 'bg-blue-900/40 text-blue-300' : 'hover:bg-gray-800 text-gray-500'}`}
              title="Grid View"
            >
              <Grid3x3 size={14} />
            </button>
            <button
              ...
              className={`p-2 rounded ${viewMode === 'compact' ? 'bg-blue-900/40 text-blue-300' : 'hover:bg-gray-800 text-gray-500'}`}
              title="Compact View"
            >
              <Minimize2 size={14} />
            </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* File List */}
        <div className={`flex-1 overflow-auto p-4 ${showInfoPanel ? 'w-3/4' : 'w-full'}`}>
          ...
          {getTotalItems() === 0 && !searchQuery && (
            <div className="text-center text-gray-500 mt-12">
              ...
              <div className="flex justify-center space-x-4">
                <button
                  onClick={() => handleNewFile()}
                  className="px-6 py-3 bg-blue-900/40 hover:bg-blue-800/40 rounded-lg text-sm text-blue-300 flex items-center space-x-2"
                >
                  <FilePlus size={16} />
                  <span>New File</span>
                </button>
                <button
                  onClick={() => handleNewFolder()}
                  className="px-6 py-3 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm text-gray-300 flex items-center space-x-2"
                >
                ...
                <button
                  onClick={handleUpload}
                  className="px-6 py-3 bg-purple-900/40 hover:bg-purple-800/40 rounded-lg text-sm text-purple-300 flex items-center space-x-2"
                >
                  <Upload size={16} />
                  <span>Upload</span>
                </button>
      ...
      {/* Status Bar */}
      <div className="px-4 py-2 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500 bg-gray-900/80">
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            <Cpu size={12} className="text-green-400" />
            <span>
              {getCurrentDirectoryItems()} items • {multiSelect.size} selected
            </span>
          </div>
          ...
          {clipboard && (
            <div className="flex items-center space-x-2 text-yellow-400">
              <Copy size={12} />
              <span>Clipboard: {clipboard.node.name}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden sm:flex items-center space-x-2">
            <button
              onClick={() => selectedNode && handleOpenInTerminal(selectedNode)}
              disabled={!selectedNode || selectedNode.type !== 'directory'}
              className="flex items-center space-x-1 text-gray-400 hover:text-white disabled:opacity-30"
          ...
          <div className="text-xs flex items-center space-x-2">
            ...
            <span className="text-blue-400">HENU OS</span>
            <span className="mx-2">•</span>
      ...
      {/* Context Menu */}
      {showContextMenu && contextNodeId && (
        <div
          className="fixed z-50 bg-gray-900 border border-gray-700 rounded-lg shadow-lg py-1 min-w-48 backdrop-blur-sm"
          style={{ 
      ...
      <div className="px-4 py-2 border-t border-gray-800 text-xs text-gray-600 bg-gray-900/90 flex items-center justify-between">
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-4">
            ...
            <span className="bg-gray-800 px-2 py-0.5 rounded">Ctrl+Click</span>
            <span className="text-gray-400">•</span>
            <span className="text-gray-400">Range:</span>
            <span className="bg-gray-800 px-2 py-0.5 rounded">Shift+Click</span>
          </div>
        </div>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-1">
            <span className="bg-gray-800 px-2 py-0.5 rounded">Ctrl+F</span>
            <span>Search</span>
          </div>
          <div className="flex items-center space-x-1">
            <span className="bg-gray-800 px-2 py-0.5 rounded">Ctrl+S</span>
            <span>Info Panel</span>
          </div>
          <div className="flex items-center space-x-1">
            <span className="bg-gray-800 px-2 py-0.5 rounded">Drag & Drop</span>
            <span>Move</span>
          </div>
        </div>
      </div>
