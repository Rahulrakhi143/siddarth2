...
import { Terminal as TerminalIcon, ChevronRight, Folder, FileText, Trash2, Plus, Search, Home, FolderOpen, File, X } from 'lucide-react';
...
export const Terminal = () => {
  ...
  const [user, setUser] = useState('henu');
  const [host, setHost] = useState('terminal');
  ...
  const commandSuggestions: CommandSuggestion[] = [
    ...
    { command: 'henu', description: 'HENU OS AI tools' },
  ];
  ...
  const executeCommand = (cmd: string) => {
    ...
    switch (command) {
      case 'help':
        output = `📋 HENU OS Terminal Commands\n${'═'.repeat(60)}\n\n`;
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950 border border-gray-800 rounded-lg overflow-hidden">
      {/* Terminal Header */}
      <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between bg-gray-900/90">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-gray-500">• {getCurrentDirectoryName()}</span>
          </div>
        </div>
        <div className="flex items-center space-x-4">
          <div className="text-xs text-gray-500 font-mono hidden md:block">
            {user}@{host}:<span className="text-yellow-400">{getCurrentDirectoryName()}</span>$
          </div>
          <button
            onClick={() => clearTerminal()}
      ...
      {/* Terminal Body */}
      <div className="flex-1 overflow-auto p-4 font-mono text-sm bg-black/60">
        {/* Welcome Message - Show only on first load */}
        {state.terminalHistory.length === 0 && (
          <div className="text-green-400 mb-6">
            <div className="text-xl mb-2">🚀 HENU OS Terminal v2.0</div>
